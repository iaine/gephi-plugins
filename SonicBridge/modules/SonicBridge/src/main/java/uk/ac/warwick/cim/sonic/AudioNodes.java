...
public class AudioNodes implements Renderer {
    ...
    public void render(Item item, RenderTarget target, PreviewProperties properties) {
        ...
        if (showNodes(properties)) {
            if (target instanceof G2DTarget) {
                renderImageProcessing((AudioItem) item, (G2DTarget) target, properties, imagesDir);
            } else if (target instanceof SVGTarget) {
                renderImageSVG((AudioItem) item, (SVGTarget) target, properties, imagesDir);
            } else if (target instanceof PDFTarget) {
                renderImagePDF((AudioItem) item, (PDFTarget) target, properties, imagesDir);
            }
        }

    }

    public void renderImageProcessing(AudioItem item, G2DTarget target, PreviewProperties properties, File directory) {
    ...
    public void renderImagePDF(AudioItem item, PDFTarget target, PreviewProperties properties, File directory) {
    ...
    public void renderImageSVG(AudioItem item, SVGTarget target, PreviewProperties properties, File directory) {
        //Params
        Float x = item.getData(NodeItem.X);
        Float y = item.getData(NodeItem.Y);
        Float size = item.getData(NodeItem.SIZE);
    ...
    private String getImageDirectory() {
        return NbPreferences.forModule(sonicBridge.class)
    ...
    public PreviewProperty[] getProperties() {
        return new PreviewProperty[]{
            ...
            NbBundle.getMessage(AudioNodes.class, IMAGE_DIRECTORY + ".name"),
            NbBundle.getMessage(AudioNodes.class, IMAGE_DIRECTORY + ".description"),
